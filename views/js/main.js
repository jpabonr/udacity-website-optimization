...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    ...
    // OPTIMIZATION: Given that dx and newwidth have the same value on every for-loop iteration these were
    // taken ouside the loop to avoid unnecessary repetition of the same calculation.
...
function updatePositions(scroll_pos) {
  frame++;
  window.performance.mark("mark_start_frame");

// OPTIMIZATION: Fetching DOM elements (pizzas to be animated) is done faster by using 
// getElementsByClassName.
  var items = document.getElementsByClassName('mover');
  for (var i = 0; i < items.length; i++) {
    var phase = Math.sin((scroll_pos / 1250) + (i % 5));
...
// OPTIMIZATION: This decouples the scroll event handler from the animation loop.  Instead of firing 
// up the updatePositions function every time there's a scroll event, the last scroll position is stored
// and the function is managed by rAF.  This known design pattern for handling animations triggered
// by scroll events was sourced from https://developer.mozilla.org/en-US/docs/Web/Events/scroll
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // OPTIMIZATION: To avoid generating (and animating later on) pizzas that are not visible on the 
  // screen, the number of pizzas generated is reduced from the orignal 200 to a more reasonable 30. 
  for (var i = 0; i < 30; i++) {
